...
int passturn(char cells[9][9], int turn, int pass[1], int end[1]) {
	...
	if (end[0] == 1) { /*barresi shart pass dar halat da jaryan boodan bazi*/
		for (m = 1; m < 9; m++) { /*halghe jagozari meghdar m va n be ezaye har 64 khane jadval*/
			for (n = 1; n < 9; n++) {
				if (pass[0] == 0) { /*halat khonsa shart pass*/
					pass[0] = (-1); /*rad shoodan yek bar nobat yek bazikon*/
				}
				else {
					pass[0] = 0; /*halat khonsa shart pass*/
				}
				...
				ch[0] = 0; /*bargardandan moteghayyer haye fahmidan anjam shodan har yek az shart haye harekhat dar 8 jahat be halat avvalieh*/
				ch[1] = 0;
				ch[2] = 0;
				ch[3] = 0;
				ch[4] = 0;
